...
const enemyWaves = [
	...
	{ time: 4, type: "sine", x: 100 },
	{ time: 5, type: "spread", x: 150 },
	{ time: 6, type: "zigzag", x: 200 },
	{ time: 7, type: "straight", x: 100 },
	{ time: 8, type: "sine", x: 250 },
	{ time: 9, type: "spread", x: 150 },
	{ time: 10, type: "zigzag", x: 50 },
	{ time: 11, type: "straight", x: 150 },
	{ time: 12, type: "spread", x: 250 },
	{ time: 13, type: "sine", x: 100 },
	{ time: 14, type: "zigzag", x: 200 },
...
const loop = (timestamp: number) => {
	...
	enemyBullets = enemyBullets.filter((b) => {
		b.update(deltaTime, player?.x, player?.y);
		return (
	...
	enemies.forEach((enemy) => {
		...
		} else if (enemy instanceof SineEnemy || enemy instanceof ZigzagEnemy) {
			if (enemy.fireTimer >= enemy.fireRate) {
				const numBullets = 8;
			...
			switch (currentPattern.type) {
				...
				case "spiral": {
					...
					for (let i = 0; i < numSpiralBullets; i++) {
						const angle = (i / numSpiralBullets) * Math.PI * 2 + rotationSpeed;
						const speedX = Math.sin(angle) * bulletSpeed;
						const speedY = Math.cos(angle) * bulletSpeed;
						enemyBullets.push(
							new Bullet({
						...
						for (let j = 0; j < 3; j++) {
							...
							enemyBullets.push(
								new Bullet({
									...
									y: boss.y + boss.height / 2,
									direction: {
										x: Math.sin(spreadAngle),
										y: Math.cos(spreadAngle),
									},
									owner: "enemy",
				...
				case "doubleSpiral": {
					const numSpiralBullets = 12;
					const rotationSpeed = boss.fireTimer * 2;
					for (let i = 0; i < numSpiralBullets; i++) {
						const angle1 = (i / numSpiralBullets) * Math.PI * 2 + rotationSpeed;
						const angle2 = (i / numSpiralBullets) * Math.PI * 2 - rotationSpeed;
						enemyBullets.push(
							new Bullet({
								x: boss.x,
								y: boss.y + boss.height / 2,
								direction: {
									x: Math.sin(angle1) * bulletSpeed,
									y: Math.cos(angle1) * bulletSpeed,
								},
								owner: "enemy",
								speed: bulletSpeed,
								size: 5,
						...
						enemyBullets.push(
							new Bullet({
								...
								y: boss.y + boss.height / 2,
								direction: {
									x: Math.sin(angle2) * bulletSpeed,
									y: Math.cos(angle2) * bulletSpeed,
								},
								owner: "enemy",
								speed: bulletSpeed,
								size: 5,
		...
		playerBullets.forEach((bullet) => {
			...
			if (boss) {
				...
				if (distance < boss.hitboxRadius + bullet.size) {
					...
					bossHealthPercentage = (boss.health / boss.maxHealth) * 100;
				}
			}
		});
		let glaze = false;
